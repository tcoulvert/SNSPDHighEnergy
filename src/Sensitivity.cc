...
Sensitivity::Sensitivity(G4String name) :
  G4CMPElectrodeSensitivity(name), primaryFileName(""), hitFileName("") {
  SetHitOutputFile(ConfigManager::GetHitOutput());
  SetPrimaryOutputFile(ConfigManager::GetPrimaryOutput());
}
...
void Sensitivity::EndOfEvent(G4HCofThisEvent* HCE) {
...
void Sensitivity::SetHitOutputFile(const G4String &fn) {
  if (hitFileName != fn) {
    if (hitOutput.is_open()) hitOutput.close();
    hitFileName = fn;
    ...
    if (!hitOutput.good()) {
      ...
      G4Exception("Sensitivity::SetHitOutputFile", "PhonSense003",
                  FatalException, msg);
      hitOutput.close();
    } else {
...
void Sensitivity::SetPrimaryOutputFile(const G4String &fn) {
  if (primaryFileName != fn) {
    ...
    if (!primaryOutput.good()) {
      ...
      G4Exception("Sensitivity::SetPrimaryOutputFile", "PhonSense003",
                  FatalException, msg);
      primaryOutput.close();
    } else {
...
G4bool Sensitivity::IsHit(const G4Step* step,
